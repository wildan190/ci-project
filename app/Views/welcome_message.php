...
<header>
    ...
    <div class="heroe">
        <h1>Panduan Pemasangan Proyek</h1>
        <h2>Flight Search Aggregation (CodeIgniter <?= CodeIgniter\CodeIgniter::CI_VERSION ?>)</h2>
    </div>

</header>

<section>
    <h1>Langkah Pemasangan</h1>
    <h2>1. Instalasi Dependensi</h2>
    <pre><code>composer install</code></pre>
    <h2>2. Konfigurasi Lingkungan</h2>
    <p>Salin file <strong>env</strong> menjadi <strong>.env</strong> lalu sesuaikan:</p>
    <pre><code>app.baseURL = 'http://localhost:8080/'
SUPPLIER_BASE_URL = 'http://localhost:8081'
AVIATIONSTACK_KEY = 'ISI_KUNCI_AVIATIONSTACK'</code></pre>
    <h2>3. Menjalankan Server</h2>
    <pre><code>php spark serve
php spark serve --port 8081</code></pre>
    <p>Server utama berjalan di <strong>http://localhost:8080</strong> dan mock suppliers di <strong>http://localhost:8081</strong>.</p>
    <h2>4. Pengujian API</h2>
    <p>Swagger UI: <a href="/docs">/docs</a></p>
    <p>OpenAPI Spec: <a href="/openapi.json">/openapi.json</a></p>
    <p>Contoh agregasi:</p>
    <pre><code>/api/flights/search?origin=CGK&destination=DPS&date=2026-02-15&limit=6</code></pre>
    <p>Contoh supplier:</p>
    <pre><code>http://localhost:8081/mock/supplierA?origin=CGK&destination=DPS&date=2026-02-15&limit=3
http://localhost:8081/mock/supplierB?from=CGK&to=DPS&date=2026-02-15&limit=3</code></pre>
    <h2>5. Opsi Tambahan</h2>
    <p>Filter: <code>airline</code>, <code>priceMin</code>, <code>priceMax</code></p>
    <p>Sort: <code>sortBy=price|departure_time|arrival_time</code>, <code>sortOrder=asc|desc</code></p>
    <p>Simulasi Supplier B: <code>supplierB_slow_ms</code>, <code>supplierB_fail</code></p>
</section>
